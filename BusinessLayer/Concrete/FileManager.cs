...
namespace BusinessLayer.Concrete
{
    ...
    {
        public byte[] giveExcel<T>(List<T> list) where T : class, new()

        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial; 
            var excelPackage=new ExcelPackage();
            var excelBlank = excelPackage.Workbook.Worksheets.Add("Work1");
            excelBlank.Cells["A1"].LoadFromCollection(list, true, OfficeOpenXml.Table.TableStyles.Light16);
            return excelPackage.GetAsByteArray();
        }

        public string givePDF<T>(List<T> list) where T : class, new()
        {
